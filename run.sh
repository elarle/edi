...
ensure_files() {
	...
	if ! [ -f "$EXCLUDE_FILE" ]; then
		echo "No exclude file under path: $EXCLUDE_FILE. Creating one"
		touch "$EXCLUDE_FILE"
	fi
}
...
compile_main_only() {
    ...
    eval "$COMPILER -DBUILD_NUMBER=$(get_build_number) $WINDOWS_OBJECT_DEPENDENCIES -c main.cpp -g3 -Wall -o .temp/o/main.o"     
